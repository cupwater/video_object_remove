...
class DAVISSeg(data.Dataset):
    def __init__(self, root, mask_dilation, segs_areas_list, resolution='480p', size=(256, 256), sample_duration=0):
        self.mask_dilation = mask_dilation
        self.sample_duration = sample_duration
        self.root = root
        self.mask_dir = root + '_mask'
        self.image_dir = root + '_frame'
        self.segs_areas_list = segs_areas_list
        self.segs_list  = [seg_area[0] for seg_area in segs_areas_list]
        self.areas_list = [seg_area[1] for seg_area in segs_areas_list]
    ...
    def __len__(self):
        return len(self.segs_areas_list)
    ...
    def __getitem__(self, index):
        ...
        info = {}
        info['name'] = video
        info['num_frames'] = len(self.segs_list[index])    
        ...
        f_list_start = self.segs_list[index][0]
        f_list_end = self.segs_list[index][-1]
        f_list = list(range(f_list_start, f_list_end ))

        start_x, start_y, end_x, end_y = self.areas_list[index]
        ...
        for f in f_list: 
            ...
            origin_img_  = cv2.imread(img_file)
            origin_imgs.append(origin_img_)
            # only use the segmented image for inpainting
            image_ = origin_img_[start_x:end_x, start_y:end_y]
            ...
            try:
                ...
                mask_ = mask_[start_x:end_x, start_y:end_y]
        ...
        return origin_imgs, images, masks, self.areas_list[index], info
